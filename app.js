...
app.get('/download', (req, res) => { // define a handler for downloading files
  ...
  if (!!userAddress
    ...
    try {
      ...
      res.send
      res.redirect('/route1?id=123');Status(403);
      return;
    }
  }
...
app.get('/history', (req, res) => {
  req.DocManager = new DocManager(req, res);
  // console.log('/history : ',req.DocManager);
...
/**
 * conversion document to pdf with this api
 */
app.post('/document/convert', (req, res) => { // define a handler for converting files
  req.DocManager = new DocManager(req, res);
  ...
  try {
    ...
    if (fileUtility.getConvertExtensionsMyApi().indexOf(fileExt) !== -1) {
...
app.get('/documents/create', (req, res) => {
  ...
  const fExt = reqFilename.split('.')[1]
  const fileName = `${new Date().getTime()}.${fExt}`;
  const uploadDirTmp = path.join(docsPath, '../storage/tmp'); // and create directory for temporary files if it doesn't exist
  req.DocManager.createDirectory(uploadDirTmp);

  fileSystem.copyFile(path.join(__dirname,`public/assets/document-templates/new/new.${fExt}`), 
    path.join(__dirname,`../storage/${fileName}`), (err) => {
    if(err) throw err;
    console.log(`${reqFilename} was copied to ${fileName}`);
    // fileSystem.rename(
    //   path.join(__dirname,`../storage/tmp/${fileName}`),
...
app.get("/models", (req, res) => {
  const docsPath = path.join(__dirname, "public/models");
  const docsPaths = fileSystem.readdirSync(docsPath);
 ...
 * create modal
 */
app.get('/create/with/model', (req, res) => {
  const reqFilename = req.query.fileName;
  const docsPath = path.join(__dirname);
  const fileName = new Date().getTime()+'.'+reqFilename.split('.')[1]
  fileSystem.copyFile(path.join(__dirname,`public/models/${reqFilename}`), path.join(__dirname,`../storage/${fileName}`), (err) => {
    if(err) throw err;
    res.send({fileName})
  })
});
 ...
 * save as model
 */
app.post('/save-as-modal', async (req, res) => {
  console.log('marche : ',req.body);
  try {
    const { status, data } = await urllib.request(req.body.url, { method: 'GET' });
    if (status === 200) {
      let fName = req.body.fileName.split('.')[0]
      const pathModal = path.join(__dirname , `public/models/${fName}.${req.body.fExt}`)
      fileSystem.writeFileSync(pathModal, data);
      res.send({status: 201, modalName: `${fName}.${req.body.fExt}`})
    } else {
      emitWarning(`Document editing service returned status:`);
    }
  } catch (error) {
